...
t=30.0  #[mm] distance to the nearest measurement point
...
d=((m*c)/cT)+t0
plt.plot([b,d],[t,t+Ames],"-om")

npnt=41;
dx=Ames/(npnt-1)
x=t+np.arange(npnt)*dx;

t_R=TOF(x,0,ht,cR)+t0
t_T0=TOF(x,0,ht,cT)+t0
t_T1=TOF(x,1,ht,cT)+t0
t_T3=TOF(x,3,ht,cT)+t0
plt.plot(t_R,x,"-k")
plt.plot(t_T0,x,"-k")
plt.plot(t_T1,x,"-k")
plt.plot(t_T3,x,"-k")
